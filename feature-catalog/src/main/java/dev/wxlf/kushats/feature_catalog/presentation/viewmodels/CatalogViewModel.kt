...
class CatalogViewModel @Inject constructor(
    ...
    private val _fetchCategoryState = MutableStateFlow<FetchCategoryUseCase.Result>(FetchCategoryUseCase.Result.Loading)
    val fetchCategoryState = _fetchCategoryState.asStateFlow()

    private val _fetchDishesState = MutableStateFlow<FetchDishesUseCase.Result>(FetchDishesUseCase.Result.Loading)
    val fetchDishesState = _fetchDishesState.asStateFlow()

    private val _addProductToBagState = MutableStateFlow<AddProductToBagUseCase.Result>(AddProductToBagUseCase.Result.Loading)
